describe Game_Message do
  ...
  before(:each) do
    ...
    LanguageFileSystem::add_dialogue("multiline", "This message\nhas several\nlines, yeah!")
  ...
  context "when a message" do
    context "contains a \dialogue[...] tag" do
      context "and the dialogue id exists" do
        ...
        it "also supports multiline dialogues" do
          ...
          expect($game_message.all_text).to eq "This message\nhas several\nlines, yeah!\n"
        end
      ...
      context "but the dialogue id doesn't exist" do
        it "displays the original message" do
          ...
          expect($game_message.all_text).to eq "I say stuff that WILL be seen\nbecause this doesn't exist: \\dialogue[wololo]\n"
        end
      end
    end
    ...
    context "doesn't contain a \dialogue[...] tag" do
      it "shows the original message" do
        ...
        expect($game_message.all_text).to eq "Is this for real?\nIs this a fantasy?\n"
      end
    end
  end
  ...
  context "when a choice" do
    ...
    context "contains a \dialogue[...] tag" do
      context "and the dialogue id exists" do
        it "shows the dialogue instead of the choice" do
          interpreter.setup_choices([["Do this!", "Or that!", "\\dialogue[hello]"], 0])

          expect($game_message.choices).to contain_exactly("Do this!", "Or that!", "Some dialogue")
        end

        it "only shows the first line of a multiline message" do
          interpreter.setup_choices([["Good choice", "Better choice", "\\dialogue[multiline]"], 0])

          expect($game_message.choices).to contain_exactly("Good choice", "Better choice", "This message")
        end
      end

      context "but the dialogue id doesn't exist" do
        it "displays the original choice" do
          interpreter.setup_choices([["A choice", "Oh another", "\\dialogue[wololo]"], 0])

          expect($game_message.choices).to contain_exactly("A choice", "Oh another", "\\dialogue[wololo]")
        end
      end
    end
    ...
    context "doesn't contain a \dialogue[...] tag" do
      it "shows the original choice" do
        ...
        expect($game_message.choices).to contain_exactly("A choice", "Oh another", "Choose me!")
      end
    end
